 ...
 * with volatile cells.
 *
 * TODO These methods should make it into imglib2-cache, so do not build
 *   build upon them remaining here forever.
 *
...
public interface Caches {
	...
	public static class RandomAccessibleLoader<T extends NativeType<T>> implements CellLoader<T> {
		...
		private BiConsumer<T, T> op = (s, t) -> t.set(s);
		...
		public void load(final SingleCellArrayImg<T, ?> cell) {
			...
			// TODO test runtime against LoopBuilder
			LoopBuilder.setImages(Views.interval(source, cell), cell).forEachPixel(op);
